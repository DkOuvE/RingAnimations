...
#define HAS_WHITE_LED (ENABLED(RGBW_LED) || ENABLED(NEOPIXEL_IS_RGBW))
...
public:
  ...
  #if ENABLED(NEOPIXEL_SLAVE_LED)
    static void set_pixel(const LEDColor &color, uint8_t p);
  
    FORCE_INLINE void set_pixel(uint8_t r, uint8_t g, uint8_t b
      #if HAS_WHITE_LED
        , uint8_t w=0
      #endif
        , uint8_t p
